...
\makeatletter
%\section 
\renewcommand{\@seccntformat}[1]{\font@pplr\selectfont\textcolor{wineRed}{\csname the#1\endcsname}\hspace{0.5em}}
...
\begin{itemize}
  \item 你无法也不需要手动把 \ci{NativeUpdateAnimation} 挪到线程池；并行是引擎的“\emph{动画评估(Evaluation) / 更新 (Update)}”阶段自动调度。
  \item \textbf{Game Thread} 负责：收集角色/移动等状态 $\rightarrow$ 拷贝到 \ci{FAnimInstanceProxy}。
  \item \textbf{Worker线程}负责：运行动画图节点更新 $+$ 姿势计算（使用 Proxy 中的只读数据副本）。
  \item \textbf{关键是}：自定义 \ci{FAnimInstanceProxy}，并保证线程安全的数据传递（值复制，不在工作线程访问 \ci{Actor/World}）。
\end{itemize}
...
\subsection{触发并行的前提}
\begin{enumerate}
  \item 组件层标志（不同版本命名略有差异），位于\cii{USkeletalMeshComponent}组件内：
  \begin{itemize}
    \item \ci{bAllowParallelUpdateAnimation = true}
    \item \ci{bParallelAnimationEvaluation = true}（或旧字段 \cii{bUseParallelEvaluation}）
  \end{itemize}
...
\textbf{特点}：
\begin{itemize}
  \item 只保存 POD \footnote{POD = “Plain Old Data”，即没有复杂构造/析构、无虚函数、标准布局、可按字节安全拷贝的老式纯数据类型；在多线程和性能敏感路径用来保证简单、可预测、低成本的数据传递。}/ 线程安全结构（\ci{float}, \ci{bool}, \ci{FVector}, \ci{FRotator}, 简单数组等）。
  \item 不持有或访问 \ci{AActor*}, \ci{UObject*} 等非线程安全对象（除非只是初始化时一次性缓存且不在 Worker 中解引用——一般仍不推荐）。
  \item 可重写 \ci{Update()}（对数据做纯数学运算）、可选 \ci{Evaluate()}（自定义姿势混合，需要熟悉 Pose Pipeline）。
\end{itemize}
...
在你的 \ci{UAnimInstBase}（见现有的 \ci{AnimInstBase.h}）添加 Proxy 支持：
...
\begin{itemize}
  ...
  \item \textbf{禁止（Worker 中做）}：访问 \ci{GetWorld()}, 执行 Trace、修改 Gameplay Ability、广播动态多播委托、访问非线程安全容器。
  \item \textbf{不要} 在 Proxy 中 \cii{new/delete UObject} 或修改全局单例。
\end{itemize}

\subsection{可覆盖的并行策略函数（可选）}
\begin{itemize}
  \item \ci{bool UAnimInstance::ShouldUseParallelUpdateAnimation() const}：强制允许/禁止并行 Update。
  \item \ci{bool USkeletalMeshComponent::CanUseParallelEvaluation() const}：组件层逻辑判定。
\end{itemize}

\subsection{验证并行是否生效}
\begin{enumerate}
  \item 运行 \ci{stat anim}，观察 “Parallel Evaluation” 任务。
  \item 用 \ci{stat taskgraph} 检查是否出现动画任务分派。
...
\begin{enumerate}
  \item 创建 \ci{FAuraAnimInstanceProxy}。
  \item 在 \ci{UAnimInstBase} 覆盖 \ci{CreateAnimInstanceProxy / DestroyAnimInstanceProxy}。
  \item 在 \ci{NativeUpdateAnimation} 读取 Movement $\rightarrow$ 同步到 Proxy。
  \item 启用组件并行动画相关标志。
  \item 使用 \ci{stat anim} 验证。
\end{enumerate}
...
\begin{itemize}
  ...
  \item \textbf{共享数据缓存}：对全局静态（只读）表格/曲线放到线程安全共享结构（例如 \ci{TSharedPtr<const FData>}），Worker 线程只读访问。
\end{itemize}
